...
function Logo() {
  return <Link href="/" className="flex items-center gap-3" aria-label={`${clinic.name} home`}>
    <span className="grid h-10 w-10 place-items-center rounded-full bg-[var(--teal)] text-white font-bold">DS</span>
    <span><b className="block font-[var(--font-heading)] text-[17px] leading-none">Dr. Shaheen&apos;s</b><small className="mt-1 block text-[9px] uppercase tracking-[.22em] text-[var(--ink-soft)]">Dental clinic</small></span>
  </Link>;
}
  ...
  return <>
    <div className="relative z-20 hidden bg-[var(--ink)] text-white lg:block">
      <div className="container flex h-9 items-center justify-between text-[11px]">
        <span className="flex items-center gap-2"><Clock3 size={13}/> Swamiji Road · Jorasirishtala · Habra</span>
        <span>{clinic.hoursDisplay} · <a className="font-bold" href={clinic.mapLink} target="_blank" rel="noreferrer">Open Maps</a></span>
      </div>
    </div>
  ...
  return <footer className="bg-[var(--ink)] pb-28 pt-20 text-white sm:pb-10">
    <div className="container">
      <div className="grid gap-12 border-b border-white/10 pb-16 md:grid-cols-[1.35fr_.8fr_.8fr_1fr]">
        <div><Logo/><p className="mt-6 max-w-sm text-sm leading-7 text-white/60">Modern family dental care with emergency support, cosmetic treatment and patient-first guidance in Habra, West Bengal.</p><div className="mt-6 flex gap-3"><span className="grid h-10 w-10 place-items-center rounded-full bg-white/10"><Instagram size={17}/></span><a href={clinic.mapLink} target="_blank" rel="noreferrer" aria-label="Open Dr. Shaheen's Dental Clinic on Google Maps" className="grid h-10 w-10 place-items-center rounded-full bg-white/10"><MapPin size={17}/></a></div></div>
      ...
      <div className="flex flex-col gap-3 pt-7 text-xs text-white/45 sm:flex-row sm:justify-between"><span>© 2026 Dr. Shaheen&apos;s Dental Clinic. All rights reserved.</span><span className="flex items-center gap-2"><ShieldCheck size={14}/> Patient-first dental care</span></div>
    </div>
  </footer>;
}
...
export function FloatingActions() {
  return <>
    <a className="fixed bottom-5 left-5 z-40 grid h-13 w-13 place-items-center rounded-full bg-[#20b45a] text-white shadow-xl sm:bottom-7 sm:left-7" href={clinic.mapLink} target="_blank" rel="noreferrer" aria-label="Open Dr. Shaheen's Dental Clinic on Google Maps"><MapPin/></a>
